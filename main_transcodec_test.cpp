...
static TimestampsTracking tt;


long long int simple_pts = 0;
...
static int open_output_file(const char* filename)
{
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO
            ...
            std::cout<<"==== Encoder name: "<<encoder->name<<std::endl;
            if (!encoder) {
...
static int encode_write_frame(unsigned int stream_index, int flush)
{
    ...
    //std::cout<<"==== filt_frame pts: "<<filt_frame->pts
    //    <<" filt_frame timebase: "<<filt_frame->time_base.den
    //    <<" | "<<filt_frame->time_base.num
    //    <<" stream: "<<stream->enc_ctx->time_base.den
    //    <<" | "<<stream->enc_ctx->time_base.num << std::endl;
    //std::cout << "==== filt_frame dts: " << filt_frame->pkt_dts << std::endl;
    ...
    while (ret >= 0) {
        ...
        //========================================================================================================
        //Messing ptk will make the timestamp incorrect reflect the actual stream time, making the video unplayable
  //      std::cout<<"==== Write packet dts and prv_dts and packet duration: "
  //          <<enc_pkt->dts<<" | "<<tt.prv_dts <<" | "<<enc_pkt->duration<<std::endl;
  //      int64_t orig_dts = enc_pkt->dts;
  //      int64_t orig_pts = enc_pkt->pts;
  //      //enc_pkt->pts += tt.pts_offset;
  //      //enc_pkt->dts += tt.dts_offset;
  //      if (enc_pkt->dts <= tt.prv_dts){
  //          std::cout <<"==== need offset dts"<<std::endl;
  //          if(tt.prv_duration == 0)
		//		enc_pkt->dts = tt.prv_dts + 1;
		//	else
		//		enc_pkt->dts = tt.prv_dts + tt.prv_duration;
  //          tt.dts_offset = enc_pkt->dts - orig_dts;
		//}
  //      if (enc_pkt->pts <= tt.prv_pts) {
  //          if(tt.prv_duration == 0)
		//		enc_pkt->pts = tt.prv_pts + 1;
		//	else
  //              enc_pkt->pts = tt.prv_pts + tt.prv_duration;
  //          tt.pts_offset = enc_pkt->pts - orig_pts;
  //      }
  //      tt.prv_pts = enc_pkt->pts;
  //      tt.prv_dts = enc_pkt->dts;
  //      tt.prv_duration = enc_pkt->duration;

  //      std::cout << "tt.offset | tt.prv_duration: " << tt.dts_offset << " | " << tt.prv_duration << std::endl;
  //      std::cout<<"==== Write packet pts and dts: "<<enc_pkt->pts<<" | "<<enc_pkt->dts << std::endl;
        //========================================================================================================
